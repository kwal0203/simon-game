...
* Milestone 2: Production deployment (03/xx/2026)
* Milestone 3: Production hardening \+ scaling (03/xx/2026)
...
| :---- | :---- | :---- | :---- | :---- | :---- |
| 1.0a | Setup directory structure | All components are given their own directory in the source code repo: database, frontend, backend, assets, tests. | None | 0.25 days | In progress |
| 1.0b | Setup dev environment/tooling | Use uv, ruff, pyright, mypy, pytest, pre-commit, loguru, pydantic,  |  | 0.25 days | In progress |
| 1.1 | Database Setup | Initialize schema and write first Alembic migration. | None | 0.5 days | To Do |
| 1.2 | API Models | Create Pydantic models for API request/response contracts. | None | 0.5 days | To Do |
| 1.3 | Read path | Write GET /v1/leaderboard | 1.1, 1.2 | 0.25 days | To Do |
| 1.4 | Write path | Write POST /v1/scores | 1.1, 1.2 | 0.25 days | To Do |
| 1.5 | React Frontend | Write the React frontend including the game play and leaderboard UI. | 1.1, 1.3 | 0.5 days | To Do |
| 1.6 | Integration | Integrate frontend, backend and database. | 1.3, 1.4, 1.5 | 0.5 days | To Do |
| 1.7 | Gameplay testing | Manual testing of gameplay running on localhost GET and POST endpoints use database appropriately Duplicate scores rejected by database | 1.1, 1.2, 1.3, 1.4 | 0.25 days | To do |
...
| Task ID | Task Name | Description | Dependencies | Estimate | Status |
| :---- | :---- | :---- | :---- | :---- | :---- |
| 2.1 | Dockerization | Write Dockerfiles for each system component and docker-compose. | 1.7 | 0.5 days | To Do |
| 2.2 | Infrastructure Setup | Provision cloud instances, configure security groups. | None | 0.5 days | To Do |
| 2.3 | CI/CD setup | Setup GitHub Actions to handle automated deployment | 2.1 | 0.5 days | To Do |
| 2.4 | Public domain | Register public domain and point A-records at EC2 elastic IP address | 2.2 | 0.5 days | To Do |
| 2.5 | Certificates | Provision Let’s Encrypt for certificate generation | 2.4 | 0.5 days | To Do |
| 2.6 | Validation | Game is reachable and playable at public URL Game only accessible on port 80 or 443 Database port 5432 not accessible Leaderboard rejects duplicate writes | 2.1 \- 2.5 | 0.5 days | To Do |
